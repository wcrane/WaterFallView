...
- (void)reloadData
{
    ...
    self.visibleCellInfo = [self getNewVisibleCellInfo];
...
- (void)didSelectCell:(EKStreamViewCell *)cell{
    if (self.delegate && [self.delegate respondsToSelector:@selector(streamView:selectedCellAtIndex:)]) {
        [self.delegate streamView:self selectedCellAtIndex:cell.index];
    }
}
...
- (void)layoutCellWithCellInfo:(EKStreamViewCellInfo *)info
{
    EKStreamViewCell *cell = [delegate streamView:self cellAtIndex:info.index];
...
{
    NSSet *newVisibleCellInfo = [streamView getNewVisibleCellInfo];
    NSSet *visibleCellInfoTmp = streamView.visibleCellInfo;
    NSMutableDictionary *cellCache = streamView.cellCache;
    
    if (visibleCellInfoTmp == nil) {
        DLog(@"nil set");
    }
    
    for (EKStreamViewCellInfo *info in visibleCellInfoTmp) {
    ...
    for (EKStreamViewCellInfo *info in newVisibleCellInfo) {
        if (![visibleCellInfoTmp containsObject:info]) {
